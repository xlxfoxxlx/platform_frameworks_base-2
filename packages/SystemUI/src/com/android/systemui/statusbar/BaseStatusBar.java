...
import com.android.internal.util.aosip.OmniSwitchConstants;
...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    protected void showRecents(boolean triggeredFromAltTab, boolean fromHome) {
        if (isOmniSwitchEnabled()) {
            if (!mScreenPinningEnabled) {
                Intent showIntent = new Intent(aosipUtils.ACTION_SHOW_OVERLAY);
    ...
    protected void hideRecents(boolean triggeredFromAltTab, boolean triggeredFromHomeKey) {
        if (isOmniSwitchEnabled()) {
            if (!mScreenPinningEnabled) {
                Intent showIntent = new Intent(aosipUtils.ACTION_HIDE_OVERLAY);
                mContext.sendBroadcastAsUser(showIntent, UserHandle.CURRENT);
            }
    ...
    protected void toggleRecents() {
        if (isOmniSwitchEnabled()) {
            if (!mScreenPinningEnabled) {
                Intent showIntent = new Intent(aosipUtils.ACTION_TOGGLE_OVERLAY);
                mContext.sendBroadcastAsUser(showIntent, UserHandle.CURRENT);
            }
        } else if (mRecents != null) {
